...
import shutil
#四肢の対称性に基づく判定
def is_facing_forward_limbs(keypoints):
    required_pairs = [
    ...
    for left_name, left_idx, right_name, right_idx in required_pairs:
        if left_idx >= len(kp) or right_idx >= len(kp):
            return False
        left_x, _, left_v = kp[left_idx]
        right_x, _, right_v = kp[right_idx]
        if left_v > 0 and right_v > 0:
            if abs(left_x - right_x) > threshold:
                return False
        else:
...
def main():
    ...
    for json_file in json_files:
        ...
        is_forward = is_facing_forward_limbs(keypoints)
